...
namespace Client.MirScenes
{
    ...
    public sealed class MapControl : MirControl
    {
        ...
        {
            var texture = DX_Slim.DX_NewTextureNoneManaged(Width, Height);
            var stream = DX_Slim.DX_TextureLockRectangle(texture);
            using (Bitmap image = new Bitmap(Width, Height, Width * 4, PixelFormat.Format32bppArgb, DX_Slim.DX_DataRectanglePointer(stream)))
            {
            ...
            DX_Slim.DX_TextureUnlockRectangle(texture);
        ...
        protected override void CreateTexture()
        {
            ...
            if (ControlTexture == null || ControlTexture.Disposed)
            {
                ...
                ControlTexture = DX_Slim.DX_NewTextureRenderTargetDefault(Size.Width, Size.Height);
                TextureSize = Size;
            }

            var oldSurface = DX_Slim.CurrentSurface;
            var surface = DX_Slim.DX_GetSurfaceLevel0(ControlTexture);
            DXManager.SetSurface(surface);
            DX_Slim.DX_DeviceClearTarget00(BackColour);

            if (FloorValid)
            {
                DXManager.Draw(DX_Slim.FloorTexture, new Rectangle(0, 0, Settings.ScreenWidth, Settings.ScreenHeight), Vector3.Zero, Color.White);
            }
        ...
        {
            if (DX_Slim.FloorTexture == null || DX_Slim.FloorTexture.Disposed)
            {
                DX_Slim.FloorTexture = DX_Slim.DX_NewTextureRenderTargetDefault(Settings.ScreenWidth, Settings.ScreenHeight);
                DX_Slim.FloorSurface = DX_Slim.DX_GetSurfaceLevel0(DX_Slim.FloorTexture);
            }

            var oldSurface = DX_Slim.CurrentSurface;
            DXManager.SetSurface(DX_Slim.FloorSurface);
            DX_Slim.DX_DeviceClearTarget00(Color.Empty); //Color.Black
        ...
        {
            if (DX_Slim.Lights == null || DX_Slim.Lights.Count == 0) return;

            if (DX_Slim.LightTexture == null || DX_Slim.LightTexture.Disposed)
            {
                DX_Slim.LightTexture = DX_Slim.DX_NewTextureRenderTargetDefault(Settings.ScreenWidth, Settings.ScreenHeight);
                DX_Slim.LightSurface = DX_Slim.DX_GetSurfaceLevel0(DX_Slim.LightTexture);
            }

            var oldSurface = DX_Slim.CurrentSurface;
            DXManager.SetSurface(DX_Slim.LightSurface);
            ...
            DX_Slim.DX_DeviceClearTarget00(darkness);
            ...
            DX_Slim.DX_DeviceSetRenderStateSourceAlphaOne();
            ...
            for (int i = 0; i < Objects.Count; i++)
            {
                ...
                if (ob.Light > 0 && (!ob.Dead || ob == MapObject.User || ob.Race == ObjectType.Spell))
                {
                    ...
                    if (lightRange >= DX_Slim.Lights.Count)
                        lightRange = DX_Slim.Lights.Count - 1;
                    ...
                    if (DX_Slim.Lights[lightRange] != null && !DX_Slim.Lights[lightRange].Disposed)
                    {
                        p.Offset(-(DXManager.LightSizes[lightRange].X / 2) - (CellWidth / 2), -(DXManager.LightSizes[lightRange].Y / 2) - (CellHeight / 2) -5);
                        DXManager.Draw(DX_Slim.Lights[lightRange], null, new Vector3((float)p.X, (float)p.Y, 0.0F), lightColour);
                    }
                }
                ...
                for (int e = 0; e < ob.Effects.Count; e++)
                {
                    ...
                    if (DX_Slim.Lights[light] != null && !DX_Slim.Lights[light].Disposed)
                    {
                        p.Offset(-(DXManager.LightSizes[light].X / 2) - (CellWidth / 2), -(DXManager.LightSizes[light].Y / 2) - (CellHeight / 2) - 5);
                        DXManager.Draw(DX_Slim.Lights[light], null, new Vector3((float)p.X, (float)p.Y, 0.0F), lightColour);
                    }
            ...
            for (int y = MapObject.User.Movement.Y - ViewRangeY - 24; y <= MapObject.User.Movement.Y + ViewRangeY + 24; y++)
            {
                ...
                for (int x = MapObject.User.Movement.X - ViewRangeX - 24; x < MapObject.User.Movement.X + ViewRangeX + 24; x++)
                {
                    ...
                    if (light >= DX_Slim.Lights.Count)
                        light = DX_Slim.Lights.Count - 1;

                    if (DX_Slim.Lights[light] != null && !DX_Slim.Lights[light].Disposed)
                    {
                        p.Offset(-(DXManager.LightSizes[light].X / 2) - (CellWidth / 2) + 10, -(DXManager.LightSizes[light].Y / 2) - (CellHeight / 2) - 5);
                        DXManager.Draw(DX_Slim.Lights[light], null, new Vector3((float)p.X, (float)p.Y, 0.0F), lightIntensity);
                    }
                }
            }
            #endregion
            ...
            DX_Slim.DX_DeviceSetRenderStateZeroSourceColor();

            DXManager.Draw(DX_Slim.LightTexture, new Rectangle(0, 0, Settings.ScreenWidth, Settings.ScreenHeight), Vector3.Zero, Color.White);

            DX_Slim.DX_SpriteEnd();
            DX_Slim.DX_SpriteBeginAlphaBlend();
        }
